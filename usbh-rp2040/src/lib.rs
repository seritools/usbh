//! USB host support for the RP2040.
//!
...
use rp2040_hal::pac::{RESETS, USBCTRL_DPRAM, USBCTRL_REGS};

const CONTROL_BUFFER_SIZE: usize = 64;

trait SubsystemReset {
    fn reset_bring_up(&self, resets: &mut rp2040_hal::pac::RESETS);
    fn reset_bring_down(&self, resets: &mut rp2040_hal::pac::RESETS);
}

macro_rules! generate_reset {
    ($MODULE:ident, $module:ident) => {
        impl SubsystemReset for ::rp2040_hal::pac::$MODULE {
            fn reset_bring_up(&self, resets: &mut ::rp2040_hal::pac::RESETS) {
    ...
    fn stop_transaction(&mut self) {
        critical_section::with(|cs| {
            ...
            inner
                .ctrl_reg
                .sie_ctrl()
                .modify(|_, w| w.stop_trans().set_bit());
        });
    }
    ...
    fn ls_preamble(&mut self, enable: bool) {
        critical_section::with(|cs| {
            ...
            inner
                .ctrl_reg
                .sie_ctrl()
                .modify(|_, w| w.preamble_en().bit(enable));
        });
    }
    ...
    fn interrupt_on_sof(&mut self, enable: bool) {
        critical_section::with(|cs| {
            ...
            inner
                .ctrl_reg
                .inte()
                .modify(|_, w| w.host_sof().bit(enable));
        });
    }
}
